...
function artistInfo(array, aname){
  const newArr = [...array]
  for(let i=0; i< newArr.length; i++){
    if(newArr[i].name === aname){
       return newArr[i].bio;
    }
    }
}
...
function artistByCountry(array, nationalityS){
  const artistNames = [];
  const newArr = [...array];
  for (let i=0; i < newArr.length; i++){
    if (newArr[i].nationality === nationalityS){
      artistNames.push(newArr[i].name)
    }
  }
  return artistNames;
}
